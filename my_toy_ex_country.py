# -*- coding: utf-8 -*-
"""
First very simple toy Unit Commitment model of... YOUR COUNTRY zone - alone (no interconnection with your neighbouring
countries) -> with PyPSA and ERAA data
-> 
...
(iii) Also, set the parameters of your country electricity generators in file
long_term_uc/toy_model_params/{your country}_parameters.py, following/adapting model of
...
uc_period_end = uc_period_start + timedelta(days=364)
# TODO: used?
...
csp_cf_data = {
    country: selec_in_df_based_on_list(df=eraa_dataset.agg_cf_data[country], selec_col=prod_type_col,
                                       selec_vals=[ProdTypeNames.csp_no_storage], rm_selec_col=True)
}
"""
...
from toy_model_params.Iberia_parameters import gps_coords
...
from toy_model_params.Iberia_parameters import get_generators, set_gen_as_list_of_gen_units_data
...
generators = get_generators(country_trigram=country_trigram, fuel_sources=fuel_sources,
                            ...
                            solar_pv_cf_data=solar_pv_cf_data[country], csp_cf_data= csp_cf_data[country])
