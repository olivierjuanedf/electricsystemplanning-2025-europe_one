...
def get_generators(country_trigram: str, fuel_sources: Dict[str, FuelSource], wind_on_shore_cf_data: pd.DataFrame,
    ...
    """
    inflow_file = pd.read_csv('/workspaces/electricsystemplanning-2025-europe_one/data/ERAA_2023-2/hydro/PECD-hydro-weekly-inflows.csv', sep=';')
    
    zone_col = inflow_file.columns[0]  # first column is zone
    clim_year = 2003                   # <-- set this to the climatic year you want

    # Filter zone

    # Ensure numeric types
    inflow_ibp = inflow_file.loc[inflow_file[zone_col] == 'iberian-peninsula'].copy()
    inflow_ibp.loc[:, 'climatic_year'] = pd.to_numeric(inflow_ibp['climatic_year'], errors='coerce')
    inflow_ibp.loc[:, 'week'] = pd.to_numeric(inflow_ibp['week'], errors='coerce')

    # Filter climatic year 2003 (NOT the country string)
    inflow_sel = inflow_ibp[inflow_ibp['climatic_year'] == clim_year].copy()
    if inflow_sel.empty:
        raise ValueError(f"No inflow rows for iberian-peninsula and climatic_year={clim_year}")

    # Sort by week
    inflow_sel = inflow_sel.sort_values(by='week')

    # Weekly inflow → daily
    inflow_col = 'cum_inflow_into_reservoirs'
    weekly_inflow = inflow_sel[inflow_col].astype(float).values
    daily_inflow = np.repeat(weekly_inflow / 7.0, 7)
    hours_per_week = 7 * 24
    hourly_inflow = np.repeat(weekly_inflow / hours_per_week, hours_per_week)  # length 52*168 = 8736
    ...
    generators = [
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 27337.56,
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 4385.61,
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 63134.5,
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.carrier: FuelNames.wind,
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 3190.0,
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 93645.51,
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 2892.2,
        ...
        },
        {
            ...
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 2048.72,
        ...
        {
            GEN_UNITS_PYPSA_PARAMS.name: f'{country_trigram}_hydro',
            GEN_UNITS_PYPSA_PARAMS.carrier: FuelNames.hydro,
            GEN_UNITS_PYPSA_PARAMS.nominal_power: 33596.57,
            GEN_UNITS_PYPSA_PARAMS.max_hours: 1059,
            GEN_UNITS_PYPSA_PARAMS.state_of_charge_initial: 0.5,
            GEN_UNITS_PYPSA_PARAMS.inflow: hourly_inflow
        },
